...
let setPoint = 30;
let u,
  e = 0; // valor de saída e valor do erro
let therms = []; // sensores
...
function startApplication() {
  ...
  app.use(routes);
  app.listen(httpPort, () =>
    console.log("Express server working at port: " + httpPort)
  );
}
...
function startSocketListening(socket) {
  ...
  socket.on("getHardwareState", () =>
    socket.emit("setHardwareState", dryerBusy)
  );
}
...
function startExperiment(controlMode) {
  ...
  fileName = `${time.getDate()}-${
    time.getMonth() + 1
  }-${time.getUTCFullYear()}-${time.getHours()}-${time.getMinutes()}-${time.getSeconds()}`;
  ...
  savingInterval = setInterval(() => {
    cmd.run(
      `echo ${getTemp()},${scale(
        u,
        "to [0,5]"
      )},${e},${setPoint} >> experiments/data/${nomeArq}.txt`
    );
  }, 250);
}
...
function startControling(mode) {
  switch (mode) {
    case "PID":
      pidControling();
      break; // controle por pid
    case "ON/OFF":
      onOffControling();
      break; // controle por liga/desliga
    case "Open loop":
      offControling(0);
      break; // controle desligado, valor constante de 3v
  }
}
...
function startSending(socket) {
  ...
  setInterval(
    () =>
      socket.emit("newData", { type: "Control", value: scale(u, "to [0,5]") }),
    500
  );
