  ...
  if (analogPin < TOTAL_ANALOG_PINS) {
    if (value == 0) {
      ...
      if (!Firmata.isResetting()) {
        ...
        // reconnecting.
        Firmata.sendAnalog(analogPin, analogRead(pin[analogPin]));
      }
    }
  }
...
void AnalogInputFirmata::report()
{
  ...
  for (pin = 0; pin < TOTAL_PINS; pin++) {
    if (IS_PIN_ANALOG(pin) && Firmata.getPinMode(pin) == PIN_MODE_ANALOG) {
      //translate physical pin to pin array index
      analogPin = PIN_TO_ANALOG(pin);
      if (analogInputsToReport & (1 << analogPin)) {
        //included by Josenalde Oliveira dec.12.2019
        //analogPin is the equivalent index within pin array, f.i., 32 is [0]
